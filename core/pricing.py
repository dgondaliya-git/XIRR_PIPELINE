...
#             else:
#                 price_date = pd.to_datetime(date, format='%d-%m-%Y').date()
#         except Exception as e:
#             if verbose:
#                 logger.warning(f"Invalid date format '{date}': {e}")
#             price_date = datetime.now().date()
...
#             if price_date in hist_dates:
#                 closing = float(hist.loc[hist_dates == price_date]["Close"].iloc[-1])
#                 if verbose:
#                     logger.info(f"✓ yfinance: {symbol} on {price_date} → ₹{closing}")
#                 return closing
#             else:
#                 earlier = hist_dates[hist_dates <= price_date]
#                 if len(earlier) > 0:
#                     best_date = earlier[-1]
#                     closing = float(hist.loc[best_date]["Close"])
#                     if verbose:
#                         delta = (price_date - best_date.date()).days
#                         logger.warning(f"⚠ yfinance: Using {best_date.date()} "
#                                      f"({delta}d earlier) → ₹{closing}")
#                     return closing
#     except Exception as e:
#         if verbose:
#             logger.error(f"✗ yfinance failed for {yf_symbol}: {e}")

#     # Try manual config as last resort
#     manual_price = get_manual_price(symbol, date)
